...
const docsBaseURL = "https://agentvault.sh/docs/guides/credential-stubs";
...
export const appRoutes = () => {
  ...
  app.get("/:provider", authMiddleware, async (c) => {
    ...
    const hint = isConfigured
      ? `Your MCP server needs local credential stub files to start. Create them in the format and location the MCP server expects, but use 'agentvault-managed' as a placeholder for all secrets. See ${docsBaseURL}/${provider}.md for examples (fallback: ${docsBaseURL}/general-app.md ). The AgentVault gateway handles real OAuth token exchange at request time.`
