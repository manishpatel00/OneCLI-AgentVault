...
description: >-
  AgentVault Gateway: transparent HTTPS proxy that injects stored credentials
  into outbound calls. Only use this skill when prompted by the gateway
  detection hook or when the user explicitly invokes /agentvault-gateway.
  Do NOT auto-load this skill based on user intent alone — the hook
  handles detection.
compatibility: Only active when HTTPS_PROXY contains aoc_ (automatic when launched via \`agentvault run\`)
metadata:
  author: agentvault
  version: "0.8.0"
---`;
...
name: agentvault-gateway
description: >-
  AgentVault Gateway: credential manager for external API access. MUST load this
...
const SKILL_BODY = `
# AgentVault Gateway — Auth Error Override
...
Your outbound HTTPS traffic is transparently proxied through the AgentVault
...
Under the AgentVault gateway, real credentials are injected at the proxy
...
3. **Create a stub file** at that exact path using \`"agentvault-managed"\` as the
...
\`\`\`json
{
  ...
  "access_token": "agentvault-managed",
  "refresh_token": "agentvault-managed",
  "client_id": "agentvault-managed",
  "client_secret": "agentvault-managed",
...
Do NOT modify or delete files containing \`"agentvault-managed"\` values.
These are managed by AgentVault.

## Credential Stubs for MCP Servers

Some MCP servers need local credential files to start. Stubs for connected
apps are pre-written automatically. Files containing \`"agentvault-managed"\`
values are managed by AgentVault — do NOT modify or delete them.

If an MCP server won't start due to missing credentials, create stubs
**before** starting it. Use \`"agentvault-managed"\` as the placeholder for all
secret values, with file permissions \`0600\`. See the guide at:
https://agentvault.sh/docs/guides/credential-stubs/general-app
...
AgentVault dashboard and connect the service there.
...
**If the error is \`claim_required\` (not \`app_not_connected\`):** the AgentVault
...
> This AgentVault project isn't claimed yet. Open this link to claim it:
...
\`"agentvault-managed"\` as the placeholder for any credential env vars or
config fields. The same error-handling rules apply: if auth fails, show
the \`connect_url\` so the user can connect the service in AgentVault.
...
- **Never** ask the user for API keys or tokens directly. Direct them to
  connect the service in the AgentVault dashboard.
