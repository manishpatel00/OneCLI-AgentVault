 ...
 *     -e POSTGRES_DB=agentvault -p 5440:5432 postgres:18-alpine
 *   DATABASE_URL="postgresql://postgres:postgres@localhost:5440/agentvault" \
 *     pnpm --filter @agentvault/db exec prisma migrate deploy
 *   POLICY_PROOF_DATABASE_URL="postgresql://postgres:postgres@localhost:5440/agentvault" \
 *     pnpm --filter @agentvault/api test -- --run src/services/app-blocklist-service.pg.test.ts
 */

const PROOF_URL = process.env.POLICY_PROOF_DATABASE_URL;

// Dynamic imports: @agentvault/db builds its client from DATABASE_URL at import
// time, so the env must be staged first.
type Db = typeof import("@agentvault/db").db;
