...
Connect an external password manager to AgentVault so the gateway can inject credentials on-demand, without storing them on the server. Currently supports [Bitwarden](https://bitwarden.com) via the [Agent Access SDK](https://github.com/bitwarden/agent-access).

## How It Works

1. You pair your Bitwarden desktop app with the AgentVault gateway (one-time setup)
...
- AgentVault running locally (`pnpm dev`) or via Docker
