...
main() {
  echo ""
  echo "  AgentVault: give your agents access, not your secrets."
  echo ""
  ...
  if [ -z "$ONECLI_BIND_HOST" ]; then
    echo "Error: Could not safely determine a bind address for AgentVault." >&2
    echo "" >&2
    echo "Please set ONECLI_BIND_HOST and try again:" >&2
    echo "  export ONECLI_BIND_HOST=<your-ip>" >&2
    echo "  curl -fsSL https://agentvault.sh/install | sh" >&2
    exit 1
  fi
  ...
  if docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" ps -q 2>/dev/null | grep -q .; then
    echo "  Stopping existing AgentVault services..."
    if ! docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" down; then
      echo "Error: Failed to stop existing AgentVault services." >&2
      exit 1
    fi
  fi
  ...
  if command -v lsof >/dev/null 2>&1; then
    if lsof -iTCP:"$PG_PORT" -sTCP:LISTEN -P -n >/dev/null 2>&1; then
      ...
      echo "" >&2
      echo "Pick a free port for AgentVault's database:" >&2
  ...
  if ! docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" pull; then
    echo "Error: Failed to pull AgentVault images. Check your network connection." >&2
    exit 1
  fi
  ...
  if ! docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" up -d --wait; then
    echo "" >&2
    echo "Error: Failed to start AgentVault." >&2
    exit 1
  fi
  ...
  echo "  To update: curl -fsSL https://agentvault.sh/install | sh"
  echo ""
}
