...
pub(super) fn proxy_auth_required() -> Response<axum::body::Body> {
    ...
    *resp.status_mut() = StatusCode::PROXY_AUTHENTICATION_REQUIRED;
    resp.headers_mut().insert(
        "proxy-authenticate",
        HeaderValue::from_static("Basic realm=\"AgentVault Gateway\""),
    );
    resp
}
...
/// Resolve the AgentVault dashboard base URL from `APP_URL`,
...
pub(super) fn bad_gateway() -> Response<axum::body::Body> {
    json_error_axum(
        ...
        serde_json::json!({
            "error": "bad_gateway",
            "message": "AgentVault gateway internal error.",
        }),
    )
}
...
pub(crate) fn app_not_connected<S>(
    ...
    with_no_retry(json_error(
        ...
        serde_json::json!({
            ...
            "message": format!("{display_name} is not connected in AgentVault. Ask the user to open this URL to connect it: {connect_url}"),
            "provider": provider,
...
pub(crate) fn access_restricted<S>(
    ...
    with_no_retry(json_error(
        ...
        serde_json::json!({
            ...
            "message": format!("{display_name} credentials exist in AgentVault but this agent does not have access. Ask the user to grant it with a policy rule: {manage_url}"),
            "provider": provider,
...
pub(crate) fn credential_not_found<S>(
    ...
    with_no_retry(json_error(
        ...
        serde_json::json!({
            ...
            "message": format!(
                "No credentials configured for {hostname} in AgentVault.\n\
...
pub(crate) fn resolution_failed<S>() -> Response<ForwardBody<S>> {
    json_error(
        ...
        serde_json::json!({
            ...
            "message": "AgentVault gateway failed to resolve rules for this request.",
        }),
    )
}
...
pub(crate) fn manual_approval_denied<S>(
    ...
    with_no_retry(json_error(
        ...
        serde_json::json!({
            ...
            "message": format!("This request was {reason} by an AgentVault manual approval policy."),
...
pub(crate) fn blocked_by_policy<S>(
    ...
    with_no_retry(json_error(
        ...
        serde_json::json!({
            ...
            "message": format!(
                "Blocked by AgentVault policy rule \"{rule_name}\". \
                 {method} {path} is not allowed. \
                 To change this, edit or disable the rule in your AgentVault dashboard."
            ),
            "rule_name": rule_name,
            "method": method,
            "path": path,
        ...
        serde_json::json!({
            ...
            "message": format!(
                "A default-deny policy blocked this request. \
                 ...
                 Create one in your AgentVault dashboard."
            ),
            "method": method,
            "host": hostname,
            "path": path,
...
pub(crate) fn rate_limited<S>(
    ...
    let mut resp = json_error(
        ...
        serde_json::json!({
            "error": "rate_limited",
            "message": "This request was rate-limited by an AgentVault policy rule.",
            "limit": limit,
            "window": window,
        }),
    );
...
pub(crate) fn approval_store_unavailable<S>() -> Response<ForwardBody<S>> {
    json_error(
        ...
        serde_json::json!({
            ...
            "message": "AgentVault manual approval service is temporarily unavailable.",
        }),
    )
}
...
mod tests {
    ...
    fn proxy_auth_required_has_correct_status_and_header() {
        ...
        assert_eq!(auth_header, "Basic realm=\"AgentVault Gateway\"");
    }
