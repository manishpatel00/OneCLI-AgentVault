...
//! whose tokens are the `"agentvault-managed"` placeholder. When the stub's
...
const ONECLI_MANAGED: &str = "agentvault-managed";
...
/// Handler for Codex refreshing its `agentvault-managed` placeholder OAuth token.
///
...
mod tests {
    ...
    #[test]
    fn codex_oauth_refresh_intercepts_agentvault_managed() {
        let body = br#"{"client_id":"app_x","grant_type":"refresh_token","refresh_token":"agentvault-managed"}"#;
        let resp = codex_oauth_refresh(body).expect("should intercept");
        assert_eq!(resp.status, StatusCode::OK);
        assert_eq!(resp.body["access_token"], "agentvault-managed");
        assert_eq!(resp.body["refresh_token"], "agentvault-managed");
    ...
    fn codex_oauth_refresh_passes_through_non_refresh_grant() {
        let body = br#"{"grant_type":"authorization_code","refresh_token":"agentvault-managed"}"#;
        assert!(codex_oauth_refresh(body).is_none());
    }
