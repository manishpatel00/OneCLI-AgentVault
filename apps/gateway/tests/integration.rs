//! Integration tests for the agentvault-gateway.
//!
...
fn start_gateway_with_envs(tmp_dir: &Path, envs: &[(&str, &str)]) -> (u16, std::process::Child) {
    ...
    let bin = env!("CARGO_BIN_EXE_agentvault-gateway");
...
/// A Codex `agentvault-managed` token refresh is answered by the gateway's default
...
#[test]
fn codex_agentvault_managed_refresh_is_intercepted() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let db_url = std::env::var("DATABASE_URL").unwrap_or_default();
    if db_url.is_empty() {
        eprintln!("skipping: DATABASE_URL not set");
    ...
    // HTTP-proxy POST (absolute URI) carrying the agentvault-managed sentinel. The
    // gateway short-circuits before forwarding, so no egress to auth.openai.com.
    let body = r#"{"grant_type":"refresh_token","refresh_token":"agentvault-managed","client_id":"app_EMoamEEZ73f0CkXaXp7hrann"}"#;
    let req = format!(
    ...
    assert!(
        resp.contains("agentvault-managed"),
        "expected synthetic agentvault-managed token body, got: {resp}"
    );
